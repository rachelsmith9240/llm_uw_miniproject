...
def cosine_similarity(x, y):
    ...
    ### TODO: Add code here ##########
    ##################################
    return math.exp(np.dot(x,y)/max(la.norm(x)*la.norm(y),1e-3))
...
def averaged_glove_embeddings_gdrive(sentence, word_index_dict, embeddings, model_type='50d'):
    ...
    # should I be manually removing punctuation? 
    for word in words:
        word_embeddings.append(get_glove_embeddings(word, word_index_dict, embeddings, model_type))
...
def get_sorted_cosine_similarity(embeddings_metadata):
    ...
    if embeddings_metadata["embedding_model"] == "glove":
        ...
        if len(st.session_state["cat_embed_" + model_name].keys()) == 0:
        ...
        for i in input_embedding:
            cosine_sim[i] = []
            for j in category_embeddings:
                print(j)
